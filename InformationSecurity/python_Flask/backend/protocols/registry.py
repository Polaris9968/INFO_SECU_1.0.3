...
PROTOCOL_PSI_MATCH = ProtocolSpec(
    ...
    has_history=True,
    has_preview_ciphertext=True,
    has_download_result=True,  # 2026-08-02 E2E fix: 与 handler fname_map 对齐 (rebase 时被 origin 侧覆盖回 False)
    has_download_result_with_original=True,
    has_download_ciphertext_by_role=False,
    has_download_original_by_role=False,
    has_download_round=True,
...
PROTOCOL_PSI_SUM = ProtocolSpec(
    ...
    has_history=True,
    has_preview_ciphertext=True,   # 2026-08-02 哥要求: 加部分密文展示 (preview-ciphertext 通用 handler 已支持)
    has_download_result=True,                 # 2026-08-01 Friday v1.1.4: PSI-Sum 加下载结果按钮
...
PROTOCOL_SS_PSI = ProtocolSpec(
    ...
    upload_data_dir_attr='SPSO_SS_PSI_DATA_DIR',
    id_length=4,
    max_members=2,  # SPIKE 5: 2-party
    runner_cls=SpsoSSPSI,
    is_mock=False,
    has_history=True,                # 2026-07-30 SPIKE 6: 哥反馈缺下一轮按钮
    has_preview_ciphertext=True,     # 2026-08-02 哥要求: 加部分密文展示
    has_download_result=False,
    has_download_result_with_original=False,
    has_download_ciphertext_by_role=False,
    has_download_original_by_role=False,
    has_download_round=True,         # 2026-07-31 哥要历史记录 UI,启用 round download endpoint
    has_demo_endpoint=True,
